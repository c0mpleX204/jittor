...
class NoisyGuidedAlphaRefineModule(TangentSpreadRefineModule):
    """
    Noisy-guided alpha refine.

    The movement direction is computed from noisy/stage-1 geometry: dense
    regions get a tangent repulsion vector, sparse regions get a tangent return
    vector from the noisy residual. The network only predicts a per-point alpha
    that decides how much of that guide to apply.
    """
    ...
    def __init__(self, model_config, transform_config):
        ...
        self.alpha_max = cfg.get("alpha_max", 1.2)
        self.guide_max_step = cfg.get("guide_max_step", self.delta_scale)
        self.residual_weight = cfg.get("residual_weight", 0.35)
        self.hole_weight = cfg.get("hole_weight", 0.65)
        self.hole_spacing_scale = cfg.get("hole_spacing_scale", 1.15)
        self.hole_power = cfg.get("hole_power", 1.0)
        self.hole_loss_weight = cfg.get("hole_loss_weight", 0.35)
        self.hole_upper_scale = cfg.get("hole_upper_scale", 1.18)
        self.guide_follow_weight = cfg.get("guide_follow_weight", 0.45)
        self.alpha_sparsity_weight = cfg.get("alpha_sparsity_weight", 0.03)
        self.alpha_feature_dim = 10

        alpha_in_dim = self.encoder.embedding_dim + self.alpha_feature_dim
        hidden = self.decoder_hidden_dim
        self.alpha_lin_1 = nn.Linear(alpha_in_dim, hidden)
        self.alpha_bn_1 = nn.BatchNorm1d(hidden)
        self.alpha_lin_2 = nn.Linear(hidden, hidden)
    ...
    def _guide_one_scale(self, pc_stage1, normal, pc_reference, k: int, max_step: float):
        ...
        zero_scalar = jt.zeros((B, N))
        if local_spacing is None:
            zero_vec = jt.zeros_like(pc_stage1)
            feature = jt.zeros((B, N, 5))
            return {
                "guide": zero_vec,
                "dense": zero_scalar,
                "sparse": zero_scalar,
                "target_spacing": jt.ones((B, N)) * 1e-3,
                "reference_spacing": jt.ones((B, N)) * 1e-3,
        ...
        dense_target = reference_spacing * float(self.reference_spacing_scale)
        dense = jt.maximum(
            (dense_target - local_spacing) / (dense_target + 1e-8),
            jt.zeros_like(local_spacing),
        )
        if self.dense_power != 1.0:
            dense = dense ** float(self.dense_power)

        sparse_target = reference_spacing * float(self.hole_spacing_scale)
        sparse = jt.maximum(
            (local_spacing - sparse_target) / (sparse_target + 1e-8),
            jt.zeros_like(local_spacing),
        )
        if self.hole_power != 1.0:
            sparse = sparse ** float(self.hole_power)

        rel = pc_stage1.unsqueeze(2) - neighbors
        bandwidth = dist[:, :, -1:] + 1e-6
        weight = jt.exp(-((dist / bandwidth) ** 2.0))
        direction = rel / (dist.unsqueeze(-1) + 1e-8)
        force = (weight.unsqueeze(-1) * direction).sum(dim=2)
        ...
        residual_step = jt.minimum(
            residual_norm,
            jt.ones_like(residual_norm) * float(max_step),
        )
        residual_delta = residual_unit * residual_step

        agreement = jt.maximum(
            (force_unit * residual_unit).sum(dim=-1),
            jt.zeros_like(local_spacing),
        )
        guide = force_unit * (float(max_step) * dense).unsqueeze(-1)
        guide = guide + (
            float(self.residual_weight) *
            (dense * agreement).unsqueeze(-1) *
            residual_delta
        )
        guide = guide + (
            float(self.hole_weight) *
            sparse.unsqueeze(-1) *
            residual_delta
        )

        cap_weight = dense + float(self.hole_weight) * sparse
        cap = (float(max_step) * cap_weight).unsqueeze(-1)
        guide_norm = jt.sqrt((guide ** 2.0).sum(dim=-1, keepdims=True) + 1e-12)
        guide = guide * jt.minimum(
            jt.ones_like(guide_norm),
            cap / (guide_norm + 1e-8),
        )

        ratio = local_spacing / (reference_spacing + 1e-8)
        residual_ratio = residual_norm.squeeze(-1) / (reference_spacing + 1e-8)
        feature = jt.concat(
            [
                dense.unsqueeze(-1),
                sparse.unsqueeze(-1),
                ratio.unsqueeze(-1),
                residual_ratio.unsqueeze(-1),
        ...
        return {
            ...
            "dense": dense,
            "sparse": sparse,
            "target_spacing": dense_target,
            "reference_spacing": reference_spacing,
            "stage_spacing": local_spacing,
            "feature": feature,
        }

    def _build_guide(self, pc_stage1, normal, pc_reference=None):
        guide_max_step = float(self.guide_max_step)
        small = self._guide_one_scale(
            pc_stage1,
            normal,
            pc_reference,
            k=self.spread_k,
            max_step=guide_max_step,
        )

        if self.mid_spread_k and self.mid_spread_k > 0 and self.mid_force_weight > 0:
            mid = self._guide_one_scale(
                pc_stage1,
                normal,
                pc_reference,
                k=self.mid_spread_k,
            ...
            guide = small["guide"] + float(self.mid_force_weight) * mid["guide"]
            dense = jt.maximum(small["dense"], float(self.mid_force_weight) * mid["dense"])
            sparse = jt.maximum(small["sparse"], float(self.mid_force_weight) * mid["sparse"])
            feature = jt.concat([small["feature"], mid["feature"]], dim=-1)
            boost = jt.maximum(mid["dense"], mid["sparse"])
        else:
            B, N, _ = pc_stage1.shape
            guide = small["guide"]
            dense = small["dense"]
            sparse = small["sparse"]
            feature = jt.concat([small["feature"], jt.zeros((B, N, 5))], dim=-1)
            boost = jt.zeros_like(dense)

        cap = (
            guide_max_step *
            (jt.ones_like(boost) + float(self.mid_delta_boost) * _clamp01(boost))
        )
        guide_norm = jt.sqrt((guide ** 2.0).sum(dim=-1, keepdims=True) + 1e-12)
        guide = guide * jt.minimum(
            jt.ones_like(guide_norm),
            cap.unsqueeze(-1) / (guide_norm + 1e-8),
        )
        return {
            "guide": guide,
            "dense": dense,
            "sparse": sparse,
            "target_spacing": small["target_spacing"],
            "reference_spacing": small["reference_spacing"],
            "stage_spacing": small["stage_spacing"],
            "feature": feature,
        }

    def _predict_alpha(self, pc_stage1, guide_feature):
        B, N, _ = pc_stage1.shape
        feat = self.encoder(pc_stage1)
        net = jt.concat([feat, guide_feature], dim=-1).reshape(B * N, -1)
        net = self.alpha_lin_1(net)
        net = self.alpha_bn_1(net)
        net = self.alpha_act(net)
        net = self.alpha_lin_2(net)
        net = self.alpha_bn_2(net)
        net = self.alpha_act(net)
        alpha = jt.sigmoid(self.alpha_lin_3(net)).reshape(B, N, 1)
        return alpha * float(self.alpha_max)
    ...
    def refine(self, pc_stage1, pc_normal_proxy=None, pc_reference=None):
        ...
        alpha = self._predict_alpha(pc_stage1, guide["feature"])
        delta = alpha * guide["guide"]
        if self.project_delta_to_tangent:
    ...
    def get_distribution_loss(
        ...
        pc_final, delta, guide, normal, normal_dot, alpha = self.refine(
            pc_stage1,
            pc_normal_proxy=pc_normal_proxy,
            pc_reference=pc_reference,
        )
        ...
        if idx is not None:
            ...
            pc_final_l = pc_final[:, idx, :]
            delta_l = delta[:, idx, :]
            guide_l = guide["guide"][:, idx, :]
            normal_dot_l = normal_dot[:, idx, :]
            alpha_l = alpha[:, idx, :]
            dense_l = guide["dense"][:, idx]
            sparse_l = guide["sparse"][:, idx]
            target_spacing_l = guide["target_spacing"][:, idx]
            reference_spacing_l = guide["reference_spacing"][:, idx]
        else:
            pc_stage1_l = pc_stage1
            pc_final_l = pc_final
            delta_l = delta
            ...
            alpha_l = alpha
            dense_l = guide["dense"]
            sparse_l = guide["sparse"]
            target_spacing_l = guide["target_spacing"]
            reference_spacing_l = guide["reference_spacing"]

        action_weight = _clamp01(jt.maximum(dense_l, sparse_l))
        guide_follow = _weighted_mean(
            ((delta_l - guide_l) ** 2.0).sum(dim=-1),
            action_weight,
        )
        ...
        if final_knn is not None:
            min_radius = (
                target_spacing_l.unsqueeze(-1) *
                float(self.repulsion_radius_scale)
            )
            close = jt.maximum(min_radius - final_knn, jt.zeros_like(final_knn))
            repulsion_loss = _weighted_mean(
                (close ** 2.0).mean(dim=2),
                jt.ones_like(action_weight),
            )

            final_spacing = final_knn.mean(dim=2)
            spacing_target = target_spacing_l * float(self.spacing_target_scale)
            spacing_shortfall = jt.maximum(
                spacing_target - final_spacing,
                jt.zeros_like(final_spacing),
            )
            spacing_loss = _weighted_mean(spacing_shortfall ** 2.0, dense_l)

            hole_upper = reference_spacing_l * float(self.hole_upper_scale)
            spacing_excess = jt.maximum(
                final_spacing - hole_upper,
                jt.zeros_like(final_spacing),
            )
            hole_loss = _weighted_mean(
                spacing_excess ** 2.0,
                jt.maximum(sparse_l, jt.ones_like(sparse_l) * 0.1),
            )

        normal_delta = (normal_dot_l.squeeze(-1) ** 2.0).mean()
        anchor = (delta_l ** 2.0).sum(dim=-1).mean()
        alpha_sparsity = _weighted_mean(
            alpha_l.squeeze(-1) ** 2.0,
            jt.ones_like(action_weight) - action_weight,
        )
        ...
        loss = (
            self.guide_follow_weight * guide_follow +
            self.repulsion_loss_weight * repulsion_loss +
            self.spacing_loss_weight * spacing_loss +
            self.hole_loss_weight * hole_loss +
            self.normal_delta_weight * normal_delta +
            self.anchor_loss_weight * anchor +
            self.alpha_sparsity_weight * alpha_sparsity +
            self.surface_bank_guard_weight * surface_bank_guard
        ) / self.dsm_sigma
        return loss
    ...
    def denoise_langevin_dynamics(self, pcl_noisy, num_steps: int=None):
        with jt.no_grad():
            pc_stage1 = self._run_stage1(pcl_noisy, num_steps=num_steps)
            pc_final, delta, _, _, _, _ = self.refine(
                pc_stage1,
                pc_reference=pcl_noisy,
            )
        return pc_final, delta
